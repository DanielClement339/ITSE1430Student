...
namespace PlayWithLinqs
{
    class Program
    {
        static void Main( string[] args )
        {
            ...
            var discounted = products.Where(p => p.IsDiscounted);           //lambdas always use the => sign

            var expensive = products.FirstOrDefault(p => p.Price > 100);    //finds first product greater than 100;

            var discountedExpensive = products.Where(p => {                 //statement version of a lambda
